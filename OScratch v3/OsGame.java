 ...
 * papplet que representa las funcionalidades de Processing. Toda aplicación o
 * juego tendrá un objeto de esta clase. Dentro de él se gestionarán, entre
 * otras cosas el escenario y los actores.
 *
 * @author Juan Luis Carrillo Arroyo
 * @version
 * <a href="https://sites.google.com/site/oscratchlibreria/" target="_blank">Versión
 * actual 1.0</a>
 */
public class OsGame extends PApplet {
...
//        this.papplet.registerMethod("textEvent", this);

        tf = new GTextField(this.papplet, 15, this.papplet.height - OsGame.ALTO_PIE - 32, this.papplet.width - 30, 20);
        tf.setVisible(false);
        tf.tag = "tf";
        tf.setPromptText("");
        tf.addEventHandler(this, "textEvent");
     ...
     * @return La variable o lista solicitada en caso de encontrarla o null si
     * no la encuentra.
     */
    ...
    /**
     * Inserta una variable o una lista en el juego siempre y cuando no haya una
     * de semenjantes características ya creadad.
     *
    ...
    /**
     * Incrementa el valor numérico de una variable del entorno del juego. En el
     * caso de que la variable no tuviera un valor numérico se le pone el valor
...
//        setFocoEnTexto();
    ...
    /**
     * Devuelve una referencia del actor a partir del nombre del mismo. Si no se
...
//        papplet.requestFocus();
     ...
     */
    private void paKeyPressed(KeyEvent evt) {
        int keyChar;
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paKeyReleased(KeyEvent evt) {
        for (int i = 0; i < pressedKeys.size(); i++) {
    ...
    public void mouseEvent(MouseEvent event) {
        switch (event.getAction()) {
            case MouseEvent.PRESS:
                paMousePressed(event);
                break;
            case MouseEvent.RELEASE:
                paMouseReleased(event);
                break;
            case MouseEvent.CLICK:
                paMouseClicked(event);
                break;
            case MouseEvent.DRAG:
                paMouseDragged(event);
                break;
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paMouseClicked(MouseEvent evt) {
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paMousePressed(MouseEvent evt) {
        for (int i = sprites.size() - 1; i >= 0; i--) {
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paMouseReleased(MouseEvent evt) {
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paMouseDragged(MouseEvent evt) {
        if (spriteDragged != null) {
     ...
     * @param evt Evento que origina la ejecución de la función.
     */
    private void paMouseMoved(MouseEvent evt) {
        if (spriteStop != null) {
            if (spriteStop.clicked(evt.getX(), evt.getY())) {
    ...
    public void textEvent(GTextField textControl, GEvent event) {
...
////        papplet.requestFocus();
//    }
    private void tfActionPerformed() {
        if (!hayHilosActivos()) {
            return;
        }
...
//        papplet.requestFocus();
    ...
    /**
     * Establece el foco en el cuadro de diálogo que se utiliza para obtener la
     * respuesta del usuario del juego frente a una pregunta.
     */
